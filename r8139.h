...
#define RTL_REG_TXCONFIG 0x40 /* Transmit Configuration Register (TCR) */
#define RTL_REG_RCR 0x44 /* Receive Configuration Register */
#define RTL_REG_9346CR 0x50
#define RTL_REG_BMCR \
	0x62 /* Basic Mode Control Register (internal PHY, MII-compatible) */
#define RTL_REG_BMSR \
	0x64 /* Basic Mode Status Register (internal PHY, MII-compatible) */

#define TXCONFIG_CLRABT (1 << 0) /* Clear Abort: leave Tx abort state */
...
/* TSD (Transmit Status Descriptor) Register Bit Definitions.
 * Bits 0-12 are the frame size the driver wrote; the status bits below
 * are filled in by the NIC once it has finished (successfully or not)
 * with the slot. */
#define TSD_OWN \
	(1 << 13) /* 1: TX complete, slot owned by CPU; 0: Slot owned by NIC */
#define TSD_TUN (1 << 14) /* Transmit FIFO Underrun */
#define TSD_TOK (1 << 15) /* Transmit OK */
#define TSD_NCC_SHIFT 24 /* Number of Collisions Count (bits 24-27) */
#define TSD_NCC_MASK (0xF << TSD_NCC_SHIFT)
#define TSD_CDH (1 << 28) /* CD Heart Beat (10Mbps half-duplex only) */
#define TSD_OWC (1 << 29) /* Out of Window Collision */
#define TSD_TABT (1 << 30) /* Transmit Abort (excess collisions) */
#define TSD_CRS (1 << 31) /* Carrier Sense Lost */
...
#define INT_RX_BUFFER_OVERFLOW (1 << 4)
#define INT_LINK_CHANGE (1 << 5)
#define INT_RX_FIFO_OVERFLOW (1 << 6)
